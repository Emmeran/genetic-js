...
var Genetic = Genetic || (function(){
	...
	function Genetic() {
		...
		this.start = function() {
			...
			function iterate()
			{
				...
				var isFinished = (typeof r != "undefined" && !r) || (i == this.configuration.iterations-1) || this.stopAsap;
				...
				if (++i == this.configuration.iterations)
					return;

				setTimeout(iterate.bind(this), 0);
			}
	...
	var worker = null;
	Genetic.prototype.stopEvolving = function()
	{
		if (!worker) return;
	...
	Genetic.prototype.evolve = function(config, userData) {
		...
		blobScript += "var started = false;\n";
		blobScript += "onmessage = function(e) { if (!started) { genetic.start(); started = true; } else { genetic.stop(); } }\n";
